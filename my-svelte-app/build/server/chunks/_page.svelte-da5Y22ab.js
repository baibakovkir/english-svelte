...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</div>` : ``} ${correctForm ? `<div class="initial__tasks__correct-form"><h3 class="initial__tasks__correct-form__title" data-svelte-h="svelte-35i5ga">Закончите предложения. Используйте слова в нужной форме.</h3> <div class="initial__tasks__correct-form__boxes"><div class="initial__tasks__correct-form__box1">${each(correctForm?.box1?.split(" "), (word) => {
    return `<div class="initial__tasks__correct-form__box1__item">${escape(word)}</div>`;
  })}</div> <div class="initial__tasks__correct-form__box2">${each(correctForm?.box2?.split(" "), (word) => {
    return `<div class="initial__tasks__correct-form__box2__item">${escape(word)}</div>`;
  })}</div></div></div>` : ``}</div></section>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-da5Y22ab.js.map
